...
metrics_results.to_csv(
    "/labmed/workspace/lotta/finaletoolkit/dataframes_notebooklasso_metrics_results.csv",
    index=False,
)
...
# Pivot
pivot_df = df.pivot(index="sample", columns="bin_id", values=list(best_metrics))
pivot_df.columns = [f"{metric}_{bin_id}" for metric, bin_id in pivot_df.columns]
...
stable_lasso = LogisticRegression(
    penalty="l1", solver="liblinear", C=c_1se, max_iter=10000, random_state=42
)
...
plt.plot(
    fpr_best, tpr_best, color="red", lw=2, label=f"Best C Model (AUC = {auc_best:.3f})"
)

# Kurve 2: 1SE Model (z.B. in Grün oder Orange)
plt.plot(
    fpr_1se, tpr_1se, color="green", lw=2, label=f"1SE Model (AUC = {auc_1se:.3f})"
)
...
plt.grid(True)
plt.savefig(
    f"/labmed/workspace/lotta/finaletoolkit/outputs/plots/lasso_parameter_tuning{bin_size}.png"
)
plt.show()
...
plt.plot(
    fpr_train, tpr_train, color="blue", lw=2, label=f"Train ROC (AUC = {auc_train:.2f})"
)
plt.plot(
    fpr_test,
    tpr_test,
...
# %%
from cv_lasso_single_fold import (
    cross_validation,
...
important_features = coef_df[coef_df["Coefficient"] != 0].sort_values(
...
stable_coef_df = pd.DataFrame(
    {"Feature": X.columns, "Coefficient": stable_lasso_model.coef_[0]}
)

stable_important_features = stable_coef_df[
    stable_coef_df["Coefficient"] != 0
...
print(
    f"Difference:   {len(important_features) - len(stable_important_features)} fewer features in 1SE model"
)
...
plt.savefig(
    f"/labmed/workspace/lotta/finaletoolkit/outputs/plots/roc_curve_{bin_size}_fold.png"
)
plt.show()
...
test_results = pd.DataFrame(
    {"Sample_ID": X_test.index, "True_Label": y_test, "Probability_Cancer": y_prob_test}
)

# 3. Sort the results by probability
test_results = test_results.sort_values(
    by="Probability_Cancer", ascending=False
...
fn_proben = test_results[
    (test_results["True_Label"] == 1) & (test_results["Probability_Cancer"] < 0.3)
]

# Falsch-Positive (Gesund als Krebs vorhergesagt)
fp_proben = test_results[
    (test_results["True_Label"] == 0) & (test_results["Probability_Cancer"] > 0.7)
]
...
if not outliers_meta.empty:
    ausreisser_klinik = clinical_df.merge(
        outliers_meta[["Sample_ID", "Probability_Cancer", "True_Label"]],
        left_on="Extracted_ID",
        right_on="Sample_ID",
    )
    print(
        f"Found outliers with threshold (FN < 0.3, FP > 0.7): {len(ausreisser_klinik)}"
    )
    print(
        ausreisser_klinik[
